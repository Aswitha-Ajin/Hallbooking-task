...
module.exports.createRoombooking=async(req,res,next)=>{
     alreadyexists=(await mongo.selectedDB.collection("RoomBooking").find(
        {
            $and: [
    ...
    if(alreadyexists){
     ...
     try{
      ...
      responseInserted = await mongo.selectedDB.collection("RoomBooking").insertOne(req.body);
      res.send(responseInserted);
...
module.exports.listCustomers= async(req,res,next)=>{
    try{
        ...
        listBookedcustomers = await mongo.selectedDB.collection("RoomBooking").aggregate(
           [
               {
                   $lookup: {
                       from: 'rooms',
